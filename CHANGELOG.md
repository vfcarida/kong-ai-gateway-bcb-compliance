...
- **AI Engineering DLP Quantitative Evaluation Benchmark (EVAL-01)**: Created continuous quantitative evaluation benchmark suite (`pii-sanitizer/tests/test_ai_eval_metrics.py`) and architectural guide (`docs/guides/ai-engineering-evaluation.md`). Evaluates entity-level TP, FP, FN, precision, recall, and F1 across multi-turn Brazilian banking dialogues. Enforces strict zero-leakage guarantee (False Negative Rate = 0.00%) on regulated Modulo-11 national identifiers (CPF, CNPJ), zero false-positive rate on adversarial negative controls, and sub-millisecond P95 latency (< 0.3ms vs 15.0ms SLA budget). Brings automated test suite to **129 passing tests**.
