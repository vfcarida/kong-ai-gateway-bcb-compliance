...
- **Reversible Tokenization Vault & Egress Re-Identification (FEAT-05)**: Added bidirectional token vault (`pii-sanitizer/app/token_vault.py`) and `POST /re-identify` endpoint (aliased to `/de-anonymize`) in `pii-sanitizer/app/main.py`. Enables authorized banking microservices to restore authentic sensitive PII from placeholder tokens or synthetic values across multi-turn LLM completions. Documented in [ADR 0004](docs/adr/0004-reversible-token-vault-re-identification.md) with 6 new pytest tests.
- **Production Kubernetes Helm Chart (FEAT-06)**: Added official Helm 3 chart (`charts/kong-ai-gateway-bcb-compliance/`) providing configurable, multi-environment deployment for Kubernetes clusters (EKS, AKS, GKE, OpenShift). Includes `Chart.yaml`, `values.yaml`, and modular templates (`_helpers.tpl`, `deployment-sanitizer.yaml`, `service-sanitizer.yaml`, `kongplugin.yaml`, `ingress.yaml`, `otel-collector.yaml`) with 3 new automated validation tests.
- **AI Engineering DLP Quantitative Evaluation Benchmark (EVAL-01)**: Created continuous quantitative evaluation benchmark suite (`pii-sanitizer/tests/test_ai_eval_metrics.py`) and architectural guide (`docs/guides/ai-engineering-evaluation.md`). Evaluates entity-level TP, FP, FN, precision, recall, and F1 across multi-turn Brazilian banking dialogues. Enforces strict zero-leakage guarantee (False Negative Rate = 0.00%) on regulated national identifiers (CPF, CNPJ, and Payment Cards/PAN), zero false-positive rate on adversarial negative controls, and sub-millisecond P95 latency (< 0.2ms vs 15.0ms SLA budget).
- **Payment Card (PAN) Detection & ISO/IEC 7812 Luhn Checksum**: Added detection and mathematical verification for 15-16 digit payment card numbers (Visa, Mastercard, Elo, Amex) using the Luhn Mod-10 checksum algorithm (`validate_luhn_checksum`). Raw numerical sequences that fail Luhn are preserved to prevent false-positive over-redaction on barcodes, tracking IDs, or timestamps. Includes format-preserving synthetic card generation (`generate_synthetic_credit_card`). Aligned with PCI-DSS v4.0 Requirement 3.3 and BCB CMN 4.893/2021 (documented in [ADR 0005](docs/adr/0005-payment-card-luhn-pci-dss-detection.md)).
- **Cloud-Native Observability & Prometheus Metrics**: Implemented standard OpenMetrics / Prometheus exposition endpoint (`GET /metrics`) in `pii-sanitizer/app/metrics.py`, recording HTTP request counts, intercepted entity types, processing latency summaries, and active token vault session gauges for Kubernetes ServiceMonitor and APM scraping.
